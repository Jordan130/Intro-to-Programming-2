...
public class BinarySearchTree<E extends Comparable<E>> {
	...
	private BTNode<E> add(E value, BTNode<E> subtreeRoot) {
		if(subtreeRoot == null) {
			...
			subtreeRoot.left = add(value, subtreeRoot.left);
			
		}
		else {
			subtreeRoot.right = add(value, subtreeRoot.right);
		}
		return subtreeRoot;
	}
	...
	private boolean contains(E value, BTNode<E> subtreeRoot) {
		...
		return contains(value, subtreeRoot.left) || contains(value, subtreeRoot.right);
	}
	 ...
	 * 
	 * @param value The element to remove.
	 * @param subtreeRoot The root of a subtree in which the element should be removed.
	...
	private BTNode<E> remove(E value, BTNode<E> subtreeRoot) {
		...
		if (value.compareTo(subtreeRoot.data) == 0) {
			if (subtreeRoot.left  ==  null && subtreeRoot.right  ==  null) {
				...
				E largest = findLargest(subtreeRoot.left);
	...
	private E findLargest(BTNode<E> subtreeRoot) {
		...
		return findLargest(subtreeRoot.right);
	}
	...
	/**
	 * Finds the number of characters in the string representation of the largest element in a subtree.
	 * 
	 * @param subtreeRoot The root of the subtree.
	 ...
	 * @postcondition toAdd has been appended to builder, as well as enough spaces on either
	 *    side to take up at least fieldWidth characters.
	 */
	...
	/**
	 * Appends information about all nodes on a certain level of the tree to a StringBuilder.
	 * 
	 ...
	 * @postcondition Information about the requested part of the tree has been appended to the StringBuilder.
	 */
	private void appendLevelString(StringBuilder builder, BTNode<E> subtreeRoot, int rootLevel, int targetLevel, int spacing, int totalHeight) {
